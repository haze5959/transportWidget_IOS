...
static const NSTimeInterval CSToastDefaultDuration  = 2.0;   //3.0;      // 안드로이드와 동일한 시간으로 맞춤
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    // 토스트를 구성할 정보들을 담을 Parent view
    UIView *wrapperView = [[UIView alloc] init];
    wrapperView.autoresizingMask = (UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin);
    wrapperView.layer.cornerRadius = CSToastCornerRadius;
    
    // 새도우, 백그라운드 색상 제거 (2016.12.21)
    // shadow
    /*
     if (CSToastDisplayShadow) {
    ...
    if (message != nil) {
        ...
        messageLabel.font = [UIFont fontWithName:@"AppleSDGothicNeo-Medium" size:13];
    ...
    // 토스트 배경 이미지 (2016.12.21)
    UIImageView *toastBackgroundImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0, 0.0, wrapperWidth, wrapperHeight)];
    UIImage *imageTemp = [UIImage imageNamed:@"bg_toast.png"];
    
    UIImage *resizeImage = [imageTemp resizableImageWithCapInsets:UIEdgeInsetsMake((imageTemp.size.height/2) - 0.1
                                                                               , (imageTemp.size.width/2) - 0.1
                                                                               , (imageTemp.size.height/2) - 0.1
                                                                               , (imageTemp.size.width/2) - 0.1)];
    toastBackgroundImageView.image = resizeImage;
    [wrapperView addSubview:toastBackgroundImageView];
